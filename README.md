...
| **Phase V** | **Execution closure** — make the platform actually change the network, not only preview it | **DELIVERED** — twelve reproduced defects fixed and each locked by a test (`tests/test_phase_v_safety.py`, 34 cases). The rollback plan is now *issued* to the device (it was built and discarded, so a half-configured device was reported as cleanly rolled back); allowlist matching is structural, closing a first-token bypass that accepted `ip http server` / `enable secret` / `username attacker privilege 15`; `configure terminal` and indentation-driven `exit` now reach the device so config is not typed in user EXEC mode; the IOS-XE renderer emits `ip address <ip> <dotted-mask>` (the CIDR form it produced is rejected by real IOS); preview and apply share one source of truth (`_all_commands`, previously `blocks[0]` only); `config_change` events are actually signed into the ledger (the old call raised `TypeError` every time behind an `except: pass`); `CONFIG_HIGH_RISK` needs `arm_high_risk()` after the typed BOND; and `access/mgmt_session.py` implements the real out-of-band path that `_refused_mgmt_factory` used to refuse unconditionally — management addresses come from discovery evidence only, and every session is **serial-confirmed** before a single line is sent. New: `demo --execute` (verified `COMPLETE-APPLIED`, 21/21 + 4/4 commands) and `netops-autopilot chat` (Arabic/English REPL). See `docs/phase-v-execution-closure.md`. **1269 tests.** |
| **Phase W** | **Deployable network** — make the applied result a network that actually works | **DELIVERED** — six reproduced defects fixed, each locked by a test (`tests/test_phase_w_deployable.py` 28 cases + `tests/test_answer_alignment.py` 4). The configuration is now **persisted** (`CONFIG_PERSIST` class, kept out of `CONFIG_CLASSES`; runs only after verification, never during rollback; failure ⇒ `PERSIST_FAILED`) where before no persist command existed anywhere in the repository; **DHCP pools** are emitted per internal zone with a gateway-covering exclusion range and space-separated `dns-server` (the comma-separated form IOS rejects); the **port inventory** is finally collected, so access-port assignment went from a structural 0 to 10 in the demo, in natural port order and weighted by planned zone size; post-apply **verification is now factual** — every applied line is read back from the running-config (`VERIFY_STATE_ABSENT` ⇒ rollback) instead of only comparing a hash, with coverage reported as `state_checked`/`state_absent`; and a credential refusal is no longer permanent — `_phase_access_retry()` re-probes discovered devices the operator supplies credentials for, requiring an **explicit** affirmative and announcing anything that stays excluded. Three further defects surfaced while proving these: the design announced trunks it never configured (one uplink per device, and a `.sort()` by FSM-4 grade made dead code by a `break` — while its `reason` string claimed grade-based selection); the test substrate gave `access-sw1` core-sw2's identity, so two neighbours were planned onto one physical socket; and the scripted answer list was hand-written in four places, so one new question silently shifted every answer. A fourth surfaced in the wiring: the demo answered the retry prompt and then discarded the answer, because `cli_main` passed a bound method that carries no `grant()` hook. See `docs/phase-w-deployable-network.md`. **1301 tests.** |
